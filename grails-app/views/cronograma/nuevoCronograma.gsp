...
<div class="dialog">
<a href="#" id="calc" class="btn" style="margin: 10px">Calcular asignaciones del proyecto</a>
...
<tr>
    ...
    <g:form action="nuevoCronograma" method="post" class="frm_anio">
        ...
        <th colspan="16">
            <g:select from="${mies.Anio.list()}" optionKey="id" optionValue="anio"
...
<tr>
    ...
    <g:each in="${mies.Mes.list()}" var="mes">
    ...
    <th style="width: 90px;">Monto<br> metas</th>
</tr>
</thead>
<tbody>
...
%{--<g:set var="totOtroAnioComp" value="0"></g:set>--}%
    ...
    <g:each in="${mies.MarcoLogico.findAllByMarcoLogicoAndEstado(comp,0,[sort:'id'])}" var="act" status="i">
        <g:set var="monto" value="${act.totalMetasAnio(anio)}"></g:set>
    ...
    %{--<g:set var="totOtrosAnios" value="0"></g:set>--}%
        <tr>
            ...
            <g:each in="${mies.Mes.list()}" var="mes" status="k">
                <g:set var="crga" value='${mies.Cronograma.findAllByMarcoLogicoAndMes(act,mes)}'></g:set>
...
<script type="text/javascript">
    $(function() {
        ...
        $("#calc").click(function(){
            if(confirm("Esta seguro que desea generar las asignaciones? Las asignaciones del proyecto generadas anteriormente seran ELIMINADAS! asi como sus DISTRIBUCIONES, PAC y PROGRAMACION.")){
                if(confirm("Los datos borrados no podrán ser recuperados!. Esta acción sera registrada en log del sistema junto con su usuario")){
                    $("#load").dialog("open")
                    $.ajax({
                        type: "POST",
